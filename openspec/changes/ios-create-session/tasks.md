...
- [x] 2.2 Refactor `AgentSessionViewModel`: optional `session`/`socket`, injected `makeSocket` factory, `draft: NewSessionDraft?`, `sessionSummaryStore` dep, `isDraftMode`; guard all `session.id`/`socket` uses (`applySyncResponse`, `applyAgentFinish`, `applyUserMessage`, `acceptOptimisticUserMessage`, `loadCachedMessages`, `markReadIfNeeded`)
...
- [x] 2.4 Draft branch in `submitDraft()`: shared optimistic steps first, then `draft.createSession(...)` → `adoptCreatedSession(response)` (mint `SessionSummary` with response id/title + draft repo fields, `sessionSummaryStore.putDisk`, `attachmentStore.adoptSessionId`, `makeSocket` + `startSocketPipeline(loadCache: false)`); failure → existing `recordSendError`. Ensure draft `canSubmitDraft` requires a selected repo, NOT `connectionState == .connected`
- [x] 2.5 Add `adoptSessionId(_:)` to `ImageAttachmentStore`
- [x] 2.6 Update `AgentSessionComponent`: `init(parent:session: SessionSummaryModel?)`, new deps on `AgentSessionDependency` (`sessionsAPI`, `reposAPI`, `modelsAPI`, `sessionSummaryStore`), build `NewSessionDraft` when session is nil, add `AgentSessionBuilder.buildNewSession()`
...
- [x] 3.2 `PromptComposerView`: add generic defaulted `@ViewBuilder trailingAccessory` slot rendered after `SendButton`
...
- [x] 3.5 Create `RepoBranchPickerBar.swift` + `RepoBranchPickerSheet.swift` — bar in own `glassBackground` rounded rect; sheet `.presentationDetents([.medium, .large])` with internal NavigationStack: searchable repo list (debounced `searchRepos`; `listRepos` page when empty) → branch list with default preselected; `// TODO: edit/plan mode toggle`
...
- [x] 4.1 `HomeRouter`: `path: [HomeDestination]` (`case session(SessionSummaryModel)`, `case newSession(id: UUID)`), update `presentationOptionsFor`/`handleNotificationTap` pattern matches, add `pushNewSession()`
...
- [ ] 5.6 Regression: existing session open/send/reconnect; notification-tap routing after the `HomeDestination` change
