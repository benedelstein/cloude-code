...
const cloudOrbs: CloudOrb[] = [
  { x: 0.12, y: 0.28, radius: 120, phase: 0.2, drift: 0.7, alpha: 0.48 },
  { x: 0.24, y: 0.18, radius: 160, phase: 1.4, drift: 0.45, alpha: 0.42 },
  { x: 0.38, y: 0.34, radius: 190, phase: 2.2, drift: 0.55, alpha: 0.38 },
  { x: 0.6, y: 0.2, radius: 220, phase: 3.1, drift: 0.35, alpha: 0.34 },
  { x: 0.76, y: 0.36, radius: 180, phase: 4.4, drift: 0.5, alpha: 0.36 },
  { x: 0.9, y: 0.22, radius: 130, phase: 5.2, drift: 0.65, alpha: 0.44 },
  { x: 0.18, y: 0.68, radius: 210, phase: 2.7, drift: 0.3, alpha: 0.3 },
  { x: 0.5, y: 0.76, radius: 260, phase: 0.8, drift: 0.28, alpha: 0.28 },
  { x: 0.82, y: 0.72, radius: 210, phase: 1.9, drift: 0.38, alpha: 0.32 },
];
...
function MorphingCloudBackground() {
  ...
  useEffect(() => {
    ...
    const drawOrb = (orb: CloudOrb, time: number) => {
      ...
      const pull = pointer.active ? Math.max(0, 1 - distance / 420) : 0;
      const pullAngle = Math.atan2(pointerY - centerY, pointerX - centerX);
      const x = centerX + Math.cos(pullAngle) * pull * 44;
      const y = centerY + Math.sin(pullAngle) * pull * 34;
      const radius = orb.radius * (1 + pull * 0.24);
      const gradient = context.createRadialGradient(x, y, radius * 0.08, x, y, radius);

      gradient.addColorStop(0, `rgba(255, 255, 255, ${orb.alpha + pull * 0.18})`);
      gradient.addColorStop(0.45, `rgba(235, 244, 251, ${orb.alpha * 0.72})`);
      ...
      for (let index = 0; index <= points; index += 1) {
        ...
        const wobble =
          ...
          pull * Math.cos(angle - pullAngle) * 0.22;
    ...
    const draw = (timeStamp: number) => {
      ...
      skyGradient.addColorStop(0.42, "#ebf4fb");
      ...
      glow.addColorStop(0, "rgba(107, 167, 213, 0.18)");
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <section className="relative isolate min-h-screen">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_15%,rgba(255,255,255,0.72),rgba(255,255,255,0.28)_38%,rgba(255,255,255,0)_68%)]" />
        ...
        <div className="relative z-10 mx-auto flex min-h-screen max-w-6xl flex-col justify-center px-6 pb-20 pt-32">
          <div className="max-w-4xl">
            <p className="mb-6 inline-flex rounded-full border border-accent/20 bg-white/54 px-4 py-2 text-sm font-medium text-accent shadow-[0_14px_50px_rgba(107,167,213,0.16)] backdrop-blur-lg">
            ...
            <p className="mt-8 max-w-2xl text-pretty text-xl leading-8 text-foreground-muted sm:text-2xl sm:leading-9">
              Agents should not borrow your laptop. Give them a whole computer
              in the cloud and let them build like developers do.
            </p>
      ...
      >
        <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-3">
          {proofPoints.map((point) => (
            <article
              key={point.title}
              className="group rounded-[2rem] border border-border bg-background-secondary/70 p-8 shadow-[0_24px_80px_rgba(15,23,42,0.06)] transition hover:-translate-y-2 hover:border-accent/40 hover:bg-white"
            >
