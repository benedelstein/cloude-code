...
Microsoft's Teams Outgoing Webhook docs say Teams sends an HTTP POST to the configured callback URL,
authenticates it with an `Authorization: HMAC <signature>` header, and expects a synchronous JSON message
response. The Worker validates the HMAC-SHA256 signature using the secret Teams shows when the webhook is
created.
